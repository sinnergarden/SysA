...
resources/                          ← 全局复用资源，按 stock 或 trade_date 组织
  ...
  financial/
    raw_pdfs/{ts_code}/{report}.pdf ← 原始财报 PDF，一次下载永不复拉
...
- **财务数据**：对每只股票，确认 `financial/parsed/{ts_code}.json` 是否存在。如果不存在，从 `financial/raw_pdfs/` 下载最近 3 个完整财年年报和所有季度报 PDF，解析并提取：
