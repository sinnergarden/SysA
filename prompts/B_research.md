...
- `research/{trade_date}/data/sysq_signals/candidates.json`（SysQ 信号）
- `research/{trade_date}/data/financial/{ts_code}.json`（财报）
- `research/{trade_date}/data/news/{ts_code}.json`（新闻）
- `research/{trade_date}/data/announcements/{ts_code}.json`（公告）
...
- 财务验证：财报数据是否支持模型逻辑（support/neutral/against/insufficient）
- 催化分析：近期新闻/公告是否存在基本面催化或题材热度
- 行业支撑：该股的行业判断（引用步骤 1 的 industry_summary）
- 反证挑战：最强的反向论证和风险点
...
- 关键数据缺失（标记为 `unavailable` 超过 2 项）时，`confidence` 不得为 `high`
