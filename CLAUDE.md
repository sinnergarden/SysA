...
SysA 的使命是**接收 SysQ 的数值信号和结构化元数据（分数、因子贡献、辅助信号、模型定义），叠加外部原始材料（财报、新闻、公告），用 LLM 产出带证据链的结构化研究结论和 A/B/C/D 评级**。它定义一套 `prompts/`（研究规则） + `steps/`（task 文件） + `schemas/`（JSON 契约） + `tools/`（确定性工具），让代码 agent 按固定 7 步研究链逐票执行、全程结构化落盘。
...
SysQ 作为一个量化系统，产出数值信号和结构化元数据。SysA 填补的是量化模型做不到、需要大语言模型推理的部分。边界如下：

| 维度 | SysQ 能产出 | SysA 做（需要 LLM） |
|------|------------|-------------------|
| 股票名单与排序 | TopK 名单 + `model_score`（数值信号） | —— |
| 主信号定义 | `primary_model`（模型元数据） | **step 01** — 理解"预测 60 天收益"的业务含义 |
| 因子贡献度 | `feature_contrib.values`（数值信号）+ `method` + `universe_stats`（归因元数据） | **step 01** — 结合 method 和 universe_stats 判断贡献度高低 |
| 辅助信号 | `auxiliary_signals[]`（数值信号 + 可选归因元数据） | **step 01** — 对比主信号与辅助信号方向一致性 |
...
**关键约束**：`feature_contrib` 是结构化的（method + values + universe_stats），不是单纯的数值字典。`method` 如实说明归因方法，`universe_stats` 提供各因子在全量候选上的分布统计，否则 LLM 无法判断 0.31 是高是低。`auxiliary_signals` 只用于交叉验证，不作为独立选股依据。所有"为什么选中该股"的文字说明都必须由 SysA step 01 产出，不得由 SysQ 预生成。
...
- **SysQ 产出数值信号和结构化元数据**：`model_score`、`feature_contrib.values`（数值信号）；`primary_model`、`feature_contrib.method`、`universe_stats`（模型/归因元数据）；`auxiliary_signals[]`（可选辅助信号）；`industry`（结构化标签）。不含 per-stock NL 解释。所有"为什么选中该股票"的逐股解释必须由 SysA step 01 生成。
