...
SysA 的使命是**接收 SysQ 的融合排序信号和多模型因子归因（N 个模型 × N 套 SHAP），叠加外部原始材料（财报、新闻、公告），用 LLM 产出带证据链的结构化研究结论和 A/B/C/D 评级**。它定义一套 `prompts/`（研究规则） + `steps/`（task 文件） + `schemas/`（JSON 契约） + `tools/`（确定性工具），让代码 agent 按固定 7 步研究链逐票执行、全程结构化落盘。
...
|------|------------|-------------------|
| 股票名单与排序 | TopK 名单 + `ranking_score`（融合排序分） | —— |
| 多模型归因 | `models[]` 数组（每个模型含 score、feature_contrib、horizon、target） | **step 01** — 对比各模型的归因异同，判断长短期信号一致性 |
...
**关键约束**：每只股票的 `models[]` 包含 N 个独立模型的归因，不存在"主/辅"等级——step 01 自行对比。`feature_contrib` 是结构化的（method + values + universe_stats），`universe_stats` 用于 LLM 判断贡献度高低。`ranking_score` 仅用于排序选股，不参与归因解释。所有"为什么选中该股"的文字说明必须由 SysA step 01 产出。
...
SysQ (数值信号)                  SysA (研究框架)                  代码 Agent (执行者)
   ┌──────────────────┐       ┌──────────────────┐    读取 steps/*  ┌────────────────┐
   │ ranking_score     │       │  prompts/        │ ←───────────  │ Claude Code CLI │
   │ models[]          │task.json│  steps/        │    写入        │ Codex CLI       │
   │   ├ 60d SHAP      │────→  │  schemas/         │ ────────────→ │ 等代码 agent    │
   │   └ 180d SHAP     │       │  tools/           │                └────────────────┘
   │ industry          │       │  memory/ (自积累) │
   └──────────────────┘       └──────────────────┘
...
- **SysQ 产出排序信号和多模型归因**：`ranking_score`（融合排序分）；`models[]`（每个模型含 score + feature_contrib + horizon/target）；`industry`。不含 per-stock NL 解释。所有"为什么选中该股票"的逐股解释必须由 SysA step 01 生成。
