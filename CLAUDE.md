...
|------|------------|-------------------|
| 股票名单与排序 | TopK 名单 + `model_score`（数值） | —— |
| 模型语义 | `model_horizon` + `model_target` | **step 01** — 理解"预测 60 天收益"的业务含义，作为解释上下文 |
| 因子贡献度 | `feature_contrib.values` 数值字典 + `method` + `universe_stats` | **step 01** — 结合 method 和 universe_stats 判断贡献度高低、解释因子意义 |
...
**关键约束**：`feature_contrib` 是结构化的（method + values + 可选 universe_stats），不是单纯的数值字典。`method` 如实说明归因方法，`universe_stats` 提供各因子在全量候选上的分布统计，否则 LLM 无法判断 0.31 是高是低。所有"为什么选中该股"的文字说明都必须由 SysA step 01 产出，不得由 SysQ 预生成。
...
SysQ (数值信号)                  SysA (研究框架)                  代码 Agent (执行者)
   ...
   │   +method     │ ──────→  │  schemas/         │    写入        │ 等代码 agent    │
   │   +universe_stats│       │  tools/           │ ────────────→ │                │
   │ industry      │           │  memory/ (自积累) │               └────────────────┘
   └──────────────┘           └──────────────────┘
...
- **SysQ 产出数值信号**：`model_score`、`model_horizon`、`model_target`、`feature_contrib`（含 method + values + 可选 universe_stats）、`industry` 标签。不含 per-stock NL 解释。所有"为什么选中该股票"的逐股解释必须由 SysA step 01 生成。
