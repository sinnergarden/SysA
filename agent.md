...
`SysA` 是一个面向 `SysQ` 候选股票研究的文件系统任务链框架。它的使命是：**接收 SysQ 的数值信号和结构化元数据（分数、因子贡献、辅助信号、模型定义），叠加外部原始材料（财报、新闻、公告），用 LLM 产出带证据链的结构化研究结论。**

SysQ 产出数值信号和结构化元数据：

- 数值信号：`model_score`、`feature_contrib.values`、`auxiliary_signals[].value`
- 模型元数据：`primary_model`
- 归因元数据：`feature_contrib.method`、`universe_stats`
- 结构化标签：`industry`
...
- SysQ 给出的候选股票名单与因子贡献（数值信号 + 结构化元数据）
...
### 1. SysQ 产出（数值信号 + 结构化元数据）

- 数值信号：`universe[].model_score`、`feature_contrib.values`、`auxiliary_signals[].value`
- 模型元数据：`primary_model`（horizon / target / description），决定 universe 按哪个 score 排序
- 归因元数据：`feature_contrib.method`、`feature_contrib.universe_stats`（各因子在全量候选中的分布统计）
- 结构化标签：`universe[].industry`
- 辅助信号：`universe[].auxiliary_signals[]`，每项含 name / description / value / 可选 feature_contrib。用于 step 01 交叉验证
