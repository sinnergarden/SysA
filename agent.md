...
SysQ 产出数值信号：
- `model_score` — 模型打分
- `model_horizon` — 预测周期（如 `60d` / `180d`），供 LLM 理解 model_score 的含义
- `model_target` — 预测目标（return / rank / excess_return），供 LLM 理解 model_score 的业务含义
- `feature_contrib` — 因子贡献度。附带 `method`（归因方法）和 `universe_stats`（因子分布统计，供 LLM 判断贡献度高低）
- `industry` — 行业分类标签
...
- `model_horizon` — 预测周期（如 `60d` / `180d`），供 LLM 理解 model_score 含义
- `model_target` — 预测目标（return / rank / excess_return）
- `universe[].model_score` — 模型对这只股票的打分
- `universe[].feature_contrib.method` — 归因方法（shap / permutation_importance / linear_weight）
- `universe[].feature_contrib.values` — 因子贡献度字典，如 `{"earnings_revision": 0.31}`。纯数值，不含 NL 解释
- `universe[].feature_contrib.universe_stats` — 可选。各因子在全量候选中的分布统计（min/max/mean/p50/p75/p90），供 step 01 的 LLM 判断 0.31 是高还是低
