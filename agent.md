...
`SysA` 是一个面向 `SysQ` 候选股票研究的文件系统任务链框架。它的使命是：**接收 SysQ 的数值模型信号（分数、因子贡献），叠加外部原始材料（财报、新闻、公告），用 LLM 产出带证据链的结构化研究结论。**

SysQ 产出数值信号：

- `primary_model` — 主信号定义：horizon（如 60d）、target（return）、description（自然语言说明，供 LLM 理解业务语义）
- `universe[].model_score` — 主信号打分
- `universe[].feature_contrib` — 主信号的因子贡献度。附带 `method`（归因方法）和 `universe_stats`（分布统计）
- `universe[].auxiliary_signals[]` — 可选。辅助信号如其他时间维度的预估 z-score、概率等，用于 step 01 交叉验证
- `universe[].industry` — 行业分类标签

SysQ 不产出 per-stock natural-language explanation。所有"为什么选中该股票"的逐股解释必须由 SysA step 01 生成。

具体是把：

- SysQ 给出的候选股票名单与因子贡献（纯数值）
...
- `primary_model` — 主信号定义（`horizon`/`target`/`description`），决定 universe 按哪个 score 排序
- `universe[].model_score` — 主信号的模型打分
- `universe[].feature_contrib.method` — 归因方法（shap / permutation_importance / linear_weight）
- `universe[].feature_contrib.values` — 因子贡献度字典，纯数值
- `universe[].feature_contrib.universe_stats` — 可选。各因子在全量候选中的分布统计
- `universe[].auxiliary_signals[]` — 可选。辅助信号列表，每项含 name / description / value / 可选 feature_contrib。用于 step 01 做多模型交叉验证
- `universe[].industry` — 行业分类标签
